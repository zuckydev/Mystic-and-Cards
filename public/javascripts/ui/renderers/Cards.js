class Card {
    ...
    constructor(id, name, rarity, type, state, position) {
        this.id = id;
        this.name = name;
        this.rarity = rarity;
        this.type = type;
...
class MonsterCard extends Card {
    constructor(id, name, rarity, type, state, position, hp, attack) {
        // Call the constructor of the Card class
        super(id, name, rarity, type, state, position);
...
class ShieldCard extends Card {
    constructor(id, name, rarity, type, state, position, hp) {
        super(id, name, rarity, type, state, position)
        this.hp = hp;
    }
...
class SpellCard extends Card {
    constructor(id, name, rarity, type, state, position, attack) {
        super(id, name, rarity, type, state, position)
        this.attack = attack;
    }
...
class PlayerHand {
    ...
    createCards(cardsInfo) {
        ...
        for (let cardInfo of cardsInfo) {
            ...
            if (cardInfo.type == 1) {
                cards.push(new MonsterCard(cardInfo.id, cardInfo.name, cardInfo.rarity, cardInfo.type, cardInfo.state, cardInfo.position, cardInfo.hp, cardInfo.attack));
            }
            else if (cardInfo.type == 2) {
                cards.push(new ShieldCard(cardInfo.id, cardInfo.name, cardInfo.rarity, cardInfo.type, cardInfo.state, cardInfo.position, cardInfo.hp));
            }
            else if (cardInfo.type == 3) {
                cards.push(new SpellCard(cardInfo.id, cardInfo.name, cardInfo.rarity, cardInfo.type, cardInfo.state, cardInfo.position, cardInfo.attack));
            }
        }
        return cards;
    }
...
class PlayerBoard extends PlayerHand {
    ...
    draw() {
        ...
        // text(this.title, this.x, this.y, this.width, 400);
        for (let card of this.cards) {
            let x = 400 + (Card.width * card.position) + this.cardSpacing;
            card.draw(x, this.y);
        }
    }
}
